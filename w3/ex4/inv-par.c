...
void multiplyMatrices(int n, real **A, real **B, real **C);
/********************/
...
void showMatrix (size_t n, real **A)
{
    ...
    for (i=0; i<n; ++i)
    {
        for (j=0; j<n; ++j)
        {
            A[i][j] = ((fabs(A[i][j]) < 0.000001)? 0.000: A[i][j]);
            printf ("%.2f\t", A[i][j]);
        }
        printf ("\n");
    }
...
void decomposeLUP_par(size_t n, real **A, size_t *P)
{
    /* computes L, U, P such that A=L*U*P */
